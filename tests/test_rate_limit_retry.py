...
def test_rate_limit_uses_long_curve_and_exhausts_at_five_attempts(sleeps, monkeypatch):
    ...
    # 名义曲线 2/4/8/16s, 各乘 0.8~1.2 抖动
    for delay, base in zip(sleeps, (2.0, 4.0, 8.0, 16.0)):
...
def test_non_rate_limit_keeps_short_curve(sleeps):
    """408/409/5xx 等其余可重试错误: 行为与旧版完全一致（3 次尝试, 0.2/0.4s）。"""
    with pytest.raises(RetriesExhausted) as ei:
...
def test_curve_follows_latest_error(sleeps):
    ...
    for delay, base in zip(sleeps[1:], (4.0, 8.0, 16.0)):
        assert base * 0.8 - 1e-9 <= delay <= base * 1.2 + 1e-9
...
def test_claude_client_passes_on_retry_through(monkeypatch):
    ...
    captured = {}
    def fake_send(fn, on_retry=None):
