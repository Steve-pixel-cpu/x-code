...
- 掐断真实供应商连接: server 导入时会应用用户真实 ~/.x-code/settings.json
  （api_client 带真 key/base_url）, _real_messages 与 multi_agent 的 subagent
...
def _no_real_api():
    ...
    def _apply(key: str, url):
        ...
        # reset_to 会把 client 换回裸 SDK 客户端: 按服务端 _apply_provider_config
        # 的做法重挂镜像代理, 保证后续测试补丁 _real 的路径仍然存在
        server.api_client.client = server._LiveClientProxy(
            server.api_client.raw_client)
        server._real_messages = server.api_client.raw_client.messages
